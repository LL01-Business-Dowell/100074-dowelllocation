...
urlpatterns = [
    ...
    path('continents/<slug:username>/<slug:sessionId>/<slug:projectCode>/',
         views.ContinentList.as_view()),
    path('countries/<slug:username>/<slug:sessionId>/<slug:projectCode>/',
         views.CountryList.as_view()),
    path('regions/<slug:username>/<slug:sessionId>/<slug:projectCode>/',
         views.RegionList.as_view()),
    path('regions/', views.RegionList2.as_view()),
    # path('country/<int:pk>/', views.CountryDetail.as_view()),
    path('country/<slug:query_type>/<str:query_value>/<slug:username>/<slug:sessionId>/<slug:projectCode>/',
         views.CountryDetail.as_view()),
    path('region/<int:pk>/', views.RegionDetail.as_view()),
    path('region/<slug:query_type>/<str:query_value>/<slug:username>/<slug:sessionId>/<slug:projectCode>/',
         views.RegionDetail.as_view()),
    path('target-function/', views.target_func, name='target-funtion'),
    path('target-func/<slug:collection>/<slug:fields>/<slug:mongoId>/<str:name>/',
         views.TargetFuntion.as_view(), name='target-func'),
    ...
    path('create-continent/', loc_recs_view.create_continent,
    ...
    path('create-sub-region/', loc_recs_view.create_sub_region,
         name='create_sub_region'),
    path('my-form-test/', loc_recs_view.my_form_test, name='my-form-test'),
    path('display-req-resp/', views.DisplayReqResp.as_view(),
         name='display-req-resp'),
